    ...
    <aside className="w-10 sm:w-48 bg-gray-100/40 dark:bg-gray-800/40 flex-shrink-0 mt-7">
      <nav className="flex flex-col gap-2 p-2 sm:p-4">
        {menuItems.map((item, index) => (
          <Button onClick={() => setSelectedIndex(index)} key={index} variant="ghost" className={`w-full justify-start hover:bg-[#FE8A5B]/60 transition-transform duration-300 text-medium ${selectedIndex === index ? 'bg-[#FE8A5B]/60' : ''}`}>
            {item.icon}
