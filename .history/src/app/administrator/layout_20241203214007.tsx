...
export default async function AdministratorLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { icon: <FontAwesomeIcon icon={faListCheck as IconProp} className="w-5 h-5" />, label: "Validar Mascotas" },
    { icon: <FontAwesomeIcon icon={faUserShield as IconProp} className='w-5 h-5'/>, label: "Reportes de usuarios" },
    { icon: <FontAwesomeIcon icon={faUser as IconProp} className="w-5 h-5" />, label: "Gestion de moderadores" },
    { icon: <FontAwesomeIcon icon={faBug as IconProp} className="w-5 h-5" />, label: "Reportes de la pagina" },
