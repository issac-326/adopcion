...
export default function Header({ menuItems }: { menuItems: MenuItem[] }) {
  ...
  return (
<aside className="w-10 sm:w-60 bg-gray-100/40 dark:bg-gray-800/40 flex-shrink-0 mt-7">
  <nav className="flex flex-col gap-2 p-2 sm:p-4">
    {menuItems.map((item, index) => (
      <Button
        ...
        key={index}
        variant="ghost"
        className={`w-full justify-start hover:bg-[#fe8a5b]/60 transition-transform duration-300 text-medium break-words ${selectedIndex === index ? 'bg-[#fe8a5b] text-white hover:bg-[#fe8a5b]' : ''}`}
      >
        {item.icon}
